...
public class Player5 {
					...
					if (conti) {
						...
						if (turn == playerID) {
							...
							if (round < maxRound) {
								...
								csc.sendToServer("Player " + playerID + " has completed the game in the round "
										+ (round - 1) + " with the score: " + game.getScore());
	...
	public void readnWriteChart(String name, Rules game, int round) {
		...
		if (round >= 2) {
			chart = (round - 1) + "," + String.valueOf(playerID) + "," + name + ","
